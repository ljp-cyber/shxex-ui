<template>
	<el-form :inline="inline" :model="model" class="form-inline" :size="size" :disabled="disabled" >
		<template v-for="item in curForm">
			<el-form-item :label="item.label?item.label:item.key">
				...
				:readonly="!edit"
				:name="item.key"
				:placeholder="item.placeholder?item.placeholder:item.key"></el-input>
				<!-- 选择 -->
				<el-select
		...
		<!-- 提交 -->
		<el-form-item>
	    <el-button :disabled ="!edit" type="primary" @click="submit">提交</el-button>
	    <slot></slot>
	  </el-form-item>
	</el-form>
</template>
